    ...
    let listar = () =>{
        obtenerEstudiante(id).then(r=>{
            ...
            tr.innerHTML= `<td>${cuenta.numero}</td><td>${cuenta.cue_saldo}</td><td>${cuenta.moneda}</td><td><button class="btn btn-success btn-block" onclick="ir(${cuenta.id},${cuenta.numero})">Ir</button></td>` 
